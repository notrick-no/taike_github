...
class DefectDetectorFrame(wx.Frame):
    def __init__(self):
        ...
        # 模拟数据
        self.demo_images = ["./images/img.png"]  # 准备2张示例图片
    ...
    def _init_ui(self):
        ...
        self.video_ctrl = wx.StaticBitmap(self.video_panel)
        self._mgr.AddPane(self.video_panel, aui.AuiPaneInfo().CenterPane().Name("video_pane"))
    ...
    def _create_control_panel(self):
        ...
        # 1. 视频控制组件
        video_box = wx.StaticBox(panel, label="视频控制")
        video_sizer = wx.StaticBoxSizer(video_box, wx.VERTICAL)

        self.video_source = wx.ComboBox(panel, choices=["Camera 0", "RTSP Stream"], value="Camera 0")
        self.video_start_btn = wx.Button(panel, label="▶ 启动视频流")
        self.video_stop_btn = wx.Button(panel, label="⏹ 停止视频流")
        self.video_stop_btn.Disable()

        video_btn_sizer = wx.GridSizer(2, 2, 5, 5)
        video_btn_sizer.AddMany([
            (self.video_source, 0, wx.EXPAND),
            (wx.StaticText(panel, label=""), 0),  # 占位
            (self.video_start_btn, 0, wx.EXPAND),
            (self.video_stop_btn, 0, wx.EXPAND)
        ])
        video_sizer.Add(video_btn_sizer, 0, wx.EXPAND | wx.ALL, 5)
        ...
        param_sizer.Add(wx.StaticText(panel, label="置信度阈值:"), 0, wx.ALL, 5)
        ...
        btn_sizer.AddMany([self.start_btn, self.stop_btn, self.export_btn])


        # 整合所有组件
        vbox.Add(video_sizer, 0, wx.EXPAND | wx.ALL, 5)
    ...
    def on_video_start(self, event):
        ...
        import _thread
        # 创建子线程，按钮调用这个方法，
        _thread.start_new_thread(self._learning_product, (event,))

    def _learning_product(self):
